...
export default class SimilarProducts extends React.Component {
  ...
  render() {
    ...
    return (
      <section className="similarproducts pb-5">
          ...
          <div className="row flex-row flex-nowrap overflow-x-auto">
            {recentProduct.map((product) => (
              <div className="col-sm-4 col-md-3" key={product.id}>
                <div className="product d-flex justify-content-center align-items-center flex-column gap-2 border px-2 py-3 rounded-3 shadow-sm">
                  <div className="image">
                    <img className="" src={product.image} alt="model1" />
                  </div>
                  <div className="info">
                    <h1 className="font-sm fw-bold my-2">{product.title.split(" ").slice(0, 4).join(" ")}</h1>
